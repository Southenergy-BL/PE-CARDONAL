...
COLORES_CENTRALES = {
    'PE CARDONAL': "#01AFFF",
    'PE MANANTIALES': "#A2E2FF",
    'PE LOS CERRILLOS': '#219EBC',
    # 'OTRA CENTRAL': '#023047'
}

# Agrega aquí las potencias instaladas correspondientes
POTENCIA_INSTALADA = {
    'PE CARDONAL': 31.5,
    'PE MANANTIALES': 27,
    'PE LOS CERRILLOS': 46.0
}
...
def cargar_y_procesar_datos(ruta_archivo):
    ...
    # Extraemos automáticamente las centrales desde el diccionario
    centrales_objetivo = list(POTENCIA_INSTALADA.keys())
...
# ==========================================
st.title("Monitoreo de Portafolio de Activos Renovables: STATKRAFT")

RUTA_GEN = os.path.join("datos", "Generación 2025-Jun 2026.xlsx")
RUTA_CMG = os.path.join("datos", "CMg_Multibarra_Con_Promedios.xlsx")

try:
    df_base = cargar_y_procesar_datos(RUTA_GEN)
    hay_datos_cmg = False
    
    try:
        df_precios = cargar_cmg(RUTA_CMG)
        df_base = pd.merge(df_base, df_precios[['Fecha_Hora', 'CMg_USD']], on='Fecha_Hora', how='left')
        df_base['Ingreso_Est_USD'] = df_base['Generacion_MW'] * df_base['CMg_USD']
        hay_datos_cmg = True
    ...
    with tab_op:
        ...
        max_rampa_subida = df_rampas['Rampa_MW_min'].max() if not df_rampas.empty else 0
        max_rampa_bajada = df_rampas['Rampa_MW_min'].min() if not df_rampas.empty else 0
        
        c1, c2 = st.columns(2)
        c1.metric("Máxima Rampa de Subida (Portafolio)", f"+{formato_cl(max_rampa_subida)} MW/min")
        c2.metric("Máxima Rampa de Caída (Portafolio)", f"{formato_cl(max_rampa_bajada)} MW/min")
        ...
        with col_ch1:
            ...
            fig_area.update_layout(hovermode="x unified", yaxis_title="Generación (MWh)", title="Curva de Inyección Agregada")
        ...
        with col_ch2:
            ...
            fig_bar.update_layout(yaxis_title="Tasa Cambio (MW/min)", title="Volatilidad de Inyección (Rampas)")
    ...
    with tab_econ:
        if hay_datos_cmg:
            ...
            potencia_activa = sum([POTENCIA_INSTALADA[c] for c in centrales_seleccionadas])
            ingreso_diario_mw = (ingreso_total / dias_periodo) / potencia_activa if potencia_activa > 0 else 0
            ...
            c2.metric("Precio Capturado Global", f"{formato_cl(precio_capturado)} USD/MWh")
            c3.metric("Índice de Captura", f"{formato_cl(indice_captura)} %")
            c4.metric("Inyección a Costo Cero", f"{formato_cl(porcentaje_gen_cero)} %")
            ...
            with col_e1:
                ...
                fig_ingresos = px.pie(df_ingresos, values='Ingreso_Est_USD', names='Central', color='Central', color_discrete_map=COLORES_CENTRALES, title="Participación de Ingresos por Activo")
            ...
            with col_e2:
                ...
                for central in centrales_seleccionadas:
                    df_c = df_plot_econ[df_plot_econ['Central'] == central]
                    fig_econ.add_trace(go.Bar(x=df_c['Fecha_Hora'], y=df_c['Generacion_MW'], name=central, marker_color=COLORES_CENTRALES[central]), secondary_y=False)
    ...
    with tab_kpi:
        ...
        emisiones_evitadas = gen_total_global * 0.45 
        hogares_equivalentes = (gen_total_global * 1000) / 200  
        
        st.markdown("#### Desempeño Técnico Consolidado")
        k1, k2, k3 = st.columns(3)
        k1.metric("Generación Total Portafolio", f"{formato_cl(gen_total_global)} MWh")
        k2.metric("Hora Peak Estadística", f"{hora_peak}:00 hrs")
        k3.metric("Potencia Activa Seleccionada", f"{formato_cl(sum([POTENCIA_INSTALADA[c] for c in centrales_seleccionadas]))} MW")
        ...
        with col_k2:
            ...
            st.info(f"🌿 **{formato_cl(emisiones_evitadas)} toneladas** de CO2 desplazadas.")
            st.success(f"🏠 Energía para **{formato_cl(hogares_equivalentes)} hogares**.")
            
            fig_fc = px.bar(df_kpi, x='Central', y='Factor_Capacidad_%', color='Central', color_discrete_map=COLORES_CENTRALES, text_auto='.2f', title="Factor de Planta por Activo (%)")
    ...
    # ---------------------------------------------------------
    with tab_heat:
        st.markdown("#### Perfil Horario Agregado (Suma de Activos)")
    ...
    reporte_txt = f"""REPORTE EJECUTIVO - PORTAFOLIO DE ACTIVOS
================================================
Días Evaluados: {dias_periodo}
Centrales Incluidas: {', '.join(centrales_seleccionadas)}
...
Inyección a Costo Cero: {formato_cl(porcentaje_gen_cero)} %
"""
    
    reporte_txt += f"""
--- IMPACTO AMBIENTAL (ESG) ---
Emisiones Desplazadas: {formato_cl(emisiones_evitadas)} tCO2
Hogares Equivalentes: {formato_cl(hogares_equivalentes)}
...
"""
  ...
  - HEPC: {formato_cl(row['HEPC'])} horas
  - Máxima Inyección Horaria: {formato_cl(row['Max_Generacion'])} MW\n"""

    st.sidebar.download_button("📄 Descargar Reporte Consolidado (TXT)", data=reporte_txt, file_name="Reporte_Portafolio.txt", mime="text/plain")
    
    csv_data = df_completo.to_csv(index=False, sep=';', decimal=',')
    st.sidebar.download_button("📊 Descargar Base de Datos (CSV)", data=csv_data, file_name="Data_Portafolio_Consolidada.csv", mime="text/csv")

except Exception as e:
    st.error(f"Error crítico al iniciar la plataforma. Detalle del error: {e}")